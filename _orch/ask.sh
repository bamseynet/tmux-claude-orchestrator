...
msg="$*"
[ -n "$msg" ] || { echo "usage: ask.sh <worker-id> <question>"; exit 1; }
S="$SESSION_NAME"

if ! tmux list-windows -t "$S" -F '#{window_name}' 2>/dev/null | grep -qx "$id"; then
  echo "no worker window '$id' in session '$S'"; exit 1
fi

wait_ready "$S:$id" 30 || echo "(warning: $id looks busy; asking anyway)"
send_prompt "$S:$id" "$msg"

if ! wait_ready "$S:$id" "${ORCH_ASK_TIMEOUT:-120}"; then
  echo "(warning: $id did not return to ready within ${ORCH_ASK_TIMEOUT:-120}s; showing pane as-is)"
fi
